...
class ConfiguracionController extends Controller
{
    ...
    public function index()
    {
        ...
        //TODO: cambiar la bd para que soporte por separado titulo, subtitulo, fecha y descripcion
        try{
    ...
    public function update(Request $request , $id)
    {
        ...
        $versiculos = (!empty($request->post('versiculos')))?$request->post('versiculos'):$this->_versiculos;
        ...
        if($mostrar_descripcion_video==1){
            ...
            $parametros = ["{TITULO}","{VERSICULOS}","{FECHA}","{CONTENIDO_DESCRIPCION}"];
            $datos_pdf = array($titulo,$fecha,$versiculos,$descripcion);
            $html_descripcion = str_replace($parametros,$datos_pdf,$this->_html_descripcion);

            $tamano_columnas=7;
        }
        
        $configuracion->contenido   = $this->revisarString($video_base);
        $configuracion->descripcion = $this->revisarString($html_descripcion);
